...
// Fix: parseVlanFromMacOutput per formato "12/-/-" (VLAN/VSI/BD)
...
const newCode = `function parseVlanFromMacOutput(text) {
  ...
  for (const line of lines) {
    ...
    // Pattern 2: Formato Huawei VLAN/VSI/BD
    // "0000-481a-c6f1 12/-/-                            GE3/0/16"
    // La VLAN è seguita da /-/- o simili
    const huaweiVsiBdMatch = line.match(/[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}\\s+(\\d+)\\/-\\/-/i);
    if (huaweiVsiBdMatch) return Number(huaweiVsiBdMatch[1]);

    // Pattern 3: Formato tabulare Huawei semplice
    // "0000-481a-c6f1 247       GE3/0/16      dynamic"
    const huaweiMatch = line.match(/[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}\\s+(\\d+)\\s+/i);
    if (huaweiMatch) return Number(huaweiMatch[1]);

    // Pattern 4: Formato con MAC colon-separated
    const colonMatch = line.match(/[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}\\s+(\\d+)\\s+/i);
    if (colonMatch) return Number(colonMatch[1]);
  }
  return null;
...
if (content.includes(oldCode)) {
  ...
  console.log('✓ Fix VLAN parser v2: gestisce formato VLAN/VSI/BD');
} else {
